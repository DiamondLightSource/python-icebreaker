...
for filename in filelist:
	...
	averaged_loc = np.zeros((lowpass.shape[0],lowpass.shape[1],num_of_segments),np.uint8)
	...
	with mrcfile.new((path1+str(filename[:-4]) +'_'+str(x_patches)+'x'+str(y_patches)+'x'+str(num_of_segments)+'flattened'+'.mrc'), overwrite=True) as out_image:
