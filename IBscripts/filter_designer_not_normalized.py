...
def lowpass (img, pixel_size, lowcut, edge_type, falloff, lowpass = True):
    ...
    if (edge_type == 'cos'):
        for i in range(falloff):
        	mask_edge = cv2.circle(mask,(ccol,crow),mask_rad+i,(np.cos(i*((3.14*0.5)/falloff))),2)
    elif (edge_type == 'linear'):
        for i in range(falloff):
        	mask_edge = cv2.circle(mask,(ccol,crow),mask_rad+i,(1-i*(1/falloff)),2)
    else:
        mask_edge = mask
    filter[:,:,0] = mask_edge
    filter[:,:,1] = mask_edge
...
def bandpass (img, pixel_size, lowcut, highcut, edge_type, falloff_in, falloff_out = 1, bandpass_type = True):
    ...
    if (edge_type == 'cos'):
        for i in range(falloff_out):
    	       bandpass_out = cv2.circle(bandpass_out,(ccol,crow),mask_rad_out+i,(np.cos(i*((3.14*0.5)/falloff_out))),2)
        for i in range(falloff_in):
            bandpass_in = cv2.circle(bandpass_in,(ccol,crow),mask_rad_in+i,(np.cos(i*((3.14*0.5)/falloff_in))),2)

    elif (edge_type == 'linear'):
        for i in range(falloff_out):
            bandpass_out = cv2.circle(bandpass_out,(ccol,crow),mask_rad_out+i,(1-i*(1/falloff_out)),2)
        for i in range(falloff_in):
            bandpass_in = cv2.circle(bandpass_in,(ccol,crow),mask_rad_in+i,(1-i*(1/falloff_in)),2)
