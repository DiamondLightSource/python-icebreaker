...
def equalize_im(img, x_patches, y_patches, num_of_segments):
    ...
    print(img.shape)
    if len(img.shape)==3:
        stack = np.zeros((img.shape[0],img.shape[1],img.shape[2]),dtype='int16')
        stack_size = img.shape[0]
    else:
        stack = np.zeros((img.shape[0],img.shape[1]),dtype='int16')
        stack_size = 1
    ...
    ##kmeans_float = KMeans(n_clusters=num_of_segments,max_iter=300,random_state=0,output_type='numpy')
        ...
        if stack_size > 1:
            stack[j,:,:] = final_image
        else: stack = final_image

    return stack#final_image
